      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div
              key={service.id}
              className="service-card bg-white rounded-xl shadow-lg p-8 text-center animate-on-scroll"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
        ...
        <motion.div 
          ...
          viewport={{ once: true }}
        >
          <a href="#apply" className="btn-animate inline-block bg-black text-white px-8 py-3 rounded-md font-medium">
            Apply Now
          </a>
        </motion.div>
      </div>
